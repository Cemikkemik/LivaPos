    ...
    var selectTableCTRL     =   function( $compile, $scope, $timeout, $http, $interval, $rootScope, $filter ) {
        ...
        **/
        ...
        $scope.selectOrderType          =   function(){
            ...
            // Auto assign order type according to available types.
            if( $scope.types.length == 1 ) {
                ...
                if( [ 'POST' ].indexOf( v2Checkout.ProcessType ) != -1 || $scope.canEditOrderType ) {
                    ...
                    $scope.canEditOrderType         =   true;
                    $scope.switchOrderType( v2Checkout.CartGastroType );
                    if( v2Checkout.CartGastroTable.length > 0 ) {
                        $scope.selectTable( v2Checkout.CartGastroTable[0] );
                        $scope.seatToUse                =   parseInt( v2Checkout.CartGastroTable[0].CURRENT_SEATS_USED );
                    }
                    setTimeout( function(){
        ...
        **/
        ...
        //     $http.get( '<?php echo site_url([ 'api', 'gastro', 'rooms' ]) . store_get_param( '?' );?>', {
        //         headers			:	{
        ...
        **/
        ...
        $scope.setAvailable         =   function( selectedtable ) {
            ...
            // NexoAPI.Bootbox().confirm( '<?php echo _s( 'Would you like to set this table as available ? This assume there is nobody at this table.', 'gastro' );?>', function( action ) {
            //     if( action ) {
        ...
        **/
        ...
        $scope.sendToKitchen    =   function(){
            ...
            NexoAPI.Bootbox().confirm({
                title       :   '<?php echo _s( 'Send to the kitchen', 'gastro' );?>',
        ...
        **/
        ...
        //         NexoAPI.Toast()( '<?php echo _s( 'You must define delivery details.', 'gastro' );?>' );
        ...
        NexoAPI.events.addFilter( 'test_order_type', function( order ){
            ...
            // if the order returned is a complete order then set table as available
            // if the orer has been paid from order history
            // then we can set the table as available.
            if( $scope.wasOpenFromTableHistory ) {
                console.log( 'setting table as available' );
                $scope.setAvailable( $scope.selectedTable );
            } else {
                console.log( 'setting table as used' );
                // if the order is placed for the first time, then we're upadting the table status.
        ...
        **/
        ...
        NexoAPI.events.addAction( 'reset_cart', function() {
            // $( '.sendToKitchenButton' ).append( $compile( '<a class="btn btn-primary" ng-click="sendToKitchen()"><i class="fa fa-cutlery"></i> <?php echo _s( 'Send to the kitchen', 'gastro' );?></a>' )( $scope ) );
        });
