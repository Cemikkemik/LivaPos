    ...
    <div class="row">
        <div class="col-lg-4" style="height:{{ contentHeight }}px;overflow-y:scroll;">
            <div class="input-group">
                <input type="text" ng-model="searchOrder" class="form-control" placeholder="<?php echo _s( 'Search order or Customer Name', 'nexo-playground-manager' );?>" aria-describedby="basic-addon1">
            ...
            <table class="table table-bordered box">
                <thead>
                    <tr>
                        <td width="100"><?php echo _s( 'Order Id', 'nexo-playground-manager' );?></td>
                        <td class="text-right"><?php echo _s( 'Customer name', 'nexo-playground-manager' );?></td>
                    </tr>
                </thead>
                <tbody>
                    <tr ng-repeat="order in orders" ng-click="editOrder( order.CODE )" ng-class="{ success : order.active }">
                        <td>{{ order.ID | fillZero: 4 }}</td>
                        <td class="text-right">{{ order.NOM }}</td>
                    </tr>
                    <tr ng-show="orders.length == 0">
                        <td colspan="2"><?php _e( 'No orders were found', 'nexo-playground-manager' );?></td>
                    </tr>
                </tbody>
            </table>
        </div>
        <div class="col-lg-5"><h4 class="text-center"><?php _e( 'Package Details', 'nexo-playground-manager' );?></h4>
        	<div class="row">
                ...
                <div class="col-lg-4">
                	<button class="btn btn-default btn-block" ng-click="printReceipt()">{{ printButtonText }}</button>
                </div>
            </div>
