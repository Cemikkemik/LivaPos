...
tendooApp.controller( 'nrPackage', [ '$scope', '$http', '$timeout', '$compile', '$interval', function( $scope, $http, $timeout, $compile, $interval ) {
    ...
    $scope.printButtonText      =   '<?php echo __( 'Print', 'nexo-playground-manager' );?>';
	...
	**/
	...
	$scope.getOrders			=	function( action ) {
		...
		if( action == 'search' ) {
            ...
            $http.post( '<?php echo site_url( array( 'rest', 'np_manager', 'orders' ) );?>/', {
                'input' :   $scope.searchOrder
            },{
    			headers			:	{
	...
	**/
    ...
    $scope.printReceipt         =   function(){
        if( angular.isUndefined( $scope.orders[ $scope.selectedOrderIndex ] ) ) {
            NexoAPI.Notify().warning( '<?php echo _s( 'Warning', 'nexo-playground-manager' );?>', '<?php echo _s( 'You can not print if you don\'t select an orders.', 'nexo-playground-manager' );?>' );
            return false;
        }

        var order_id    =   $scope.orders[ $scope.selectedOrderIndex ].ID;

        $http.get( '<?php echo site_url( array( 'dashboard', store_slug(), 'nexo', 'print', 'order_receipt' ) );?>/' + order_id + '?ignore_header=true&refresh=true',{

        }).then( function( returned ) {
            var html = $.parseHTML( returned.data );

            $( html ).find( 'head' ).remove();

            $( '.receipt-wrapper' ).html( html );

            $( '.receipt-wrapper' ).hide();

            var data    =
            '<div class="list-group">' +
                '<li class="list-group-item"><?php echo _s( 'Total Time:', 'nexo-playground-manager' );?><span class="pull-right">{{ orders[ selectedOrderIndex ].overallTime | secondsToDateTime | date : \'HH:mm:ss\' }}</span></li>' +
                '<li class="list-group-item"><?php echo __( 'Elapsed Time :', 'nexo-playground-manager' );?><span class="pull-right">{{ orders[ selectedOrderIndex ].USED_SECONDS | secondsToDateTime | date : \'HH:mm:ss\' }}</span></li>' +
                '<li class="list-group-item"><?php echo __( 'Remaining Time', 'nexo-playground-manager' );?><span class="pull-right">{{ orders[ selectedOrderIndex ].remainingTime | secondsToDateTime | date : \'HH:mm:ss\' }}</span></li>' +
            '</div>';

            $( '.hideOnPrint' ).html( data );

            $( '.hideOnPrint' ).html( $compile( $( '.hideOnPrint' ).html() )( $scope ) );

            $( '.hideOnPrint' ).removeClass( 'hideOnPrint' );

            setTimeout( function(){
                NexoAPI.PrintElement( '.receipt-wrapper' );
            }, 500 );
        });
    }
    /**
