...
tendooApp.directive( 'readyItems', function(){
     return {
          templateUrl    :    '<?php echo site_url([ 'dashboard', 'nexo-restaurant', 'templates', 'ready-items']);?>'
     }
})
...
function( $rootScope, $scope, $http, $interval, $timeout, $compile ){
     ...
     **/
     $scope.openReadyMeals    =    function(){
          ...
          $timeout( function(){
               angular.element( '.ready-meals .modal-dialog' ).css( 'width', '50%' );
     ...
     $scope.collectMeal       =    function( meal_id, index ) {
          $http.post( '<?php echo site_url([ 'rest', 'nexo_restaurant', 'collect_meal', store_get_param( '?' ) ]);?>', {
               meal_id
          }, {
               headers			:	{
