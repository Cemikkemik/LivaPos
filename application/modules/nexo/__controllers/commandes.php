	...
	**/
	...
	public function filter_edit_button( $string, $row, $edit_text, $subject ) 
	{
		...
		if ( in_array( @$nexo_order_types[ $row->TYPE ], array( $Cash ) ) ) {
			return;
		} else if( in_array( @$nexo_order_types[ $row->TYPE ], array( $Estimate ) ) ) {
			ob_start();
			?>
            <a href='<?php echo site_url( array( 'dashboard', 'nexo', 'commandes', 'lists', 'v2_checkout', $row->ID ) );?>' title='<?php echo $edit_text?> <?php echo $subject?>'>
                <span class='edit-icon fa fa-edit btn-default btn'></span>
            </a>
            <?php
			return ob_get_clean();
		} else if( @$nexo_order_types[ $row->TYPE ] ==  $Advance ) {
			ob_start();
			?>
            <a href='<?php echo site_url( array( 'dashboard', 'nexo', 'commandes', 'proceed', $row->ID ) );?>' title='<?php _e( 'Payer un commande', 'nexo' );?>'>
