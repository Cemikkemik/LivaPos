	...
	{		
		...
		if( ! $this->Cache->get( 'net_sales' ) ) {
			...
			if( $Sales ) {
				foreach( $Sales as $sale ) {
					...
					if( in_array( $sale[ 'TYPE' ], array( @$Options[ 'nexo_order_comptant' ], @$Options[ 'nexo_order_advance' ] ) ) ) {					
		...
		if( ! $this->Cache->get( 'creances' ) ) {
			...
			if( $Sales ) {
				foreach( $Sales as $sale ) {
					// Uniquement les commandes comptant et avance	
					if( in_array( $sale[ 'TYPE' ], array( @$Options[ 'nexo_order_devis' ] ) ) ) {					
						$CA			=	intval( $sale[ 'TOTAL' ] ) - ( intval( $sale[ 'RISTOURNE' ] ) + intval( $sale[ 'RABAIS' ] ) + intval( $sale[ 'REMISE' ] ) );
						$creances	+=	$CA;
					}
					if( in_array( $sale[ 'TYPE' ], array( @$Options[ 'nexo_order_advance' ] ) ) ) {					
