...
<table class="table table-bordered table-striped box">
    <thead>
        <tr>
            <td colspan="3"><?php echo __( 'Récapitulatif des recettes', 'nexo_premium' );?></td>
        </tr>
        <tr>
            <td><?php _e( 'Type de documents', 'nexo_premium' );?></td>
            <td><?php _e( 'Quantité', 'nexo_premium' );?></td>
            <td><?php echo sprintf( __( 'Chiffre d\'affaire collectif (%s)', 'nexo_premium' ), @$Options[ 'nexo_currency' ] );?></td>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td><?php _e( 'Commandes Cash', 'nexo_premium' );?></td>
            <td id="cash_nbr"></td>
            <td id="cash_amount" class="text-right"></td>
        </tr>
        <tr>
            <td><?php _e( 'Commandes Avance', 'nexo_premium' );?></td>
            <td id="avance_nbr"></td>
            <td id="avance_amount" class="text-right"></td>
        </tr>
        <tr>
            <td><?php _e( 'Commandes Devis', 'nexo_premium' );?></td>
            <td id="devis_nbr"></td>
            <td id="devis_amount" class="text-right"></td>
        </tr>
        <tr>
            <td></td>
            <td><?php _e( 'Chiffre d\'affaire journalier (Commandes Cash + Commandes Avance)', 'nexo_premium' );?></td>
            <td id="cash_avance_amount_total" class="text-right"></td>
        </tr>
        <tr>
            <td></td>
            <td><?php _e( 'Chiffre d\'affaire à recouvrer (Créances)', 'nexo_premium' );?></td>
...
<table class="table table-bordered table-striped box">
    <thead>
        <tr>
            <td colspan="3"><?php _e( 'Récapitulatif des dépenses', 'nexo_premium' );?></td>
        </tr>
        <tr>
            <td><?php _e( 'Désignation des documents', 'nexo_premium' );?></td>
            <td><?php echo sprintf( __( 'Montant (%s)', 'nexo_premium' ), @$Options[ 'nexo_currency' ] );?></td>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td><?php _e( 'Remise + Rabais + Ristourne', 'nexo_premium' );?></td>
            <td id="rrr_total_amount" class="text-right"></td>
        </tr>
        <!-- Looper les fctures en dépenses -->
        <!-- Fin loop -->
        <tr id="before_total">
            <td><?php _e( 'Total dépense', 'nexo_premium' );?></td>
...
<table class="table table-bordered table-striped box">
    <thead>
        <tr>
            <td><?php _e( 'Bilan tresorerie', 'nexo_premium' );?></td>
            <td><?php echo sprintf( __( 'Montant (%s)', 'nexo_premium' ), @$Options[ 'nexo_currency' ] );?></td>
        </tr>
    </thead>
    <tbody>
    	<!--
        <tr>
            <td><?php _e( 'Solde Initial', 'nexo_premium' );?></td>
...
var Nexo_Daily_Report	=	new function(){
	...
	this.OrderReport	=	function(){
		$.post( 
			...
			_.object( [ 'start', 'end' ], [ '<?php echo $CarbonReportDate->copy()->startOfDay();?>', '<?php echo $CarbonReportDate->copy()->endOfDay();?>' ] ),
			function( orders ) {
				...
				_.map( orders, function( value, key ) {
					...
					if( value.TYPE == <?php echo @$Options[ 'nexo_order_comptant' ];?> ) {
					...
					} else if( value.TYPE == <?php echo @$Options[ 'nexo_order_advance' ];?> ) {
					...
					} else if( value.TYPE == <?php echo @$Options[ 'nexo_order_devis' ];?> ) {
